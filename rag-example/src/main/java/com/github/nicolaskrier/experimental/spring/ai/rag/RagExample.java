...
class RagExample {
    ...
    @Bean
    @Order(1)
    ApplicationRunner dataLoaderApplicationRunner(DocumentReader documentReader, VectorStore vectorStore) {
        return _ -> {
            LOGGER.info("Loading recent popes into vector store.");
            var documents = documentReader.read();
            vectorStore.add(documents);
            LOGGER.info("{} documents corresponding to recent popes loaded into vector store.", documents.size());
        };
    }
    ...
    private void searchNextPopes(ChatClient chatClient, BeanOutputConverter<Pope> beanOutputConverter) {
        IntStream.range(0, nextSearchedPopesNumber).forEach(_ -> {
            try {
                var prompt = new Prompt(userPromptResource.getContentAsString(Charset.defaultCharset()));
